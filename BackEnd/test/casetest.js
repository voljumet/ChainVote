    ...
    it('Even number of SuperAdmin users can NOT create a case', async function () {
      ...
      it('Should deactivate case', async function () {
        await instance.createCase('fourth Case', 'Descripton',
           Math.round(new Date() / 1000) + 1, Math.round(new Date() / 1000) + 60 * 60,
           'yes', 'no', {from: accounts[9],});
        await truffleAssert.passes(instance.endVoting(4),truffleAssert.ErrorType.REVERT
           );
      });
      ...
      it('Should not delete a case that is open for voting', async function () {
        ...
        await truffleAssert.fails(instance.endVoting(1),truffleAssert.ErrorType.REVERT
           );
       });
